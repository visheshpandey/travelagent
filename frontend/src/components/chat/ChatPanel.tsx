...
export default function ChatPanel({ tripId }: Props) {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
        ...
        {open && (
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
              {!tripId && (
                <p className="text-xs text-tertiary">Generate an itinerary first to ask questions about it.</p>
              )}
              {messages.map((m, i) => (
                <div
                  key={i}
                  className={`text-sm rounded-2xl px-3 py-2 max-w-[85%] ${
                    m.role === "user"
                      ? "bg-accent/20 text-primary ml-auto rounded-br-sm"
                      : "bg-hoverwash text-primary rounded-bl-sm"
                  }`}
                >
                  {m.text}
                </div>
              ))}
              {loading && <div className="text-xs text-tertiary">Thinking…</div>}
            </div>

            <form onSubmit={handleAsk} className="p-3 border-t border-subtle flex gap-2">
              <input
                value={question}
              ...
              <button
                ...
                className="rounded-full bg-accent text-onaccent px-4 py-2 text-xs font-semibold disabled:opacity-40"
              >
                Ask
              </button>
            </form>
      ...
      <button
        onClick={() => setOpen((o) => !o)}
        className="rounded-full bg-accent text-onaccent w-14 h-14 flex items-center justify-center text-xl shadow-glow hover:brightness-110 transition"
      >
        💬
      </button>
    </div>
